...
from .order_history_model import OrderSchema
...
async def add_order(order: dict, email: str):
    new_cart = OrderSchema(
        email=email,
        order_date=str(date.today().strftime("%B %d, %Y")),
        cart_items=order["cart_items"],
    )
    new_order = await order_history_collection.insert_one(
        new_cart.model_dump(by_alias=True, exclude=["id"])
    )
